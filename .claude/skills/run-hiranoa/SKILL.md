---
name: run-hiranoa
description: Build, launch, and drive the Hiranoa Tauri desktop app (WebView2) via Chrome DevTools Protocol — take screenshots, click elements, dispatch keyboard shortcuts, evaluate JS, open files without the native file dialog. Use when asked to run, start, screenshot, or verify a UI change in this app.
---
...
`.claude/skills/run-hiranoa/driver.mjs`。
...
node .claude/skills/run-hiranoa/driver.mjs launch
# → "CDP準備完了。" が出るまで待つ（初回Rustビルドがあると数分かかることがある）

# 2. スクリーンショット
node .claude/skills/run-hiranoa/driver.mjs screenshot .claude/skills/run-hiranoa/screenshots/foo.png

# 3. ネイティブファイルダイアログを介さずファイルを開く（devビルド限定フック、後述）
node .claude/skills/run-hiranoa/driver.mjs openFile "E:/path/to/file.md"

# 4. 要素をクリック（CSSセレクタ）
node .claude/skills/run-hiranoa/driver.mjs click 'button[aria-label="設定"]'

# 5. キーボードショートカットを送る（document.bodyから bubbles:true でdispatchし、window側の
#    keydownリスナーへ届ける。"Ctrl+W" のように "+" 区切り、最後がkey値）
node .claude/skills/run-hiranoa/driver.mjs key "Ctrl+W"

# 6. 任意のJSを評価（Promiseはawaitされ、returnByValueで結果が返る）
node .claude/skills/run-hiranoa/driver.mjs eval 'document.title'

# 7. 終了（起動時に記録したPIDのプロセスツリーをtaskkillで終了）
node .claude/skills/run-hiranoa/driver.mjs quit
```

スクリーンショットは`.claude/skills/run-hiranoa/screenshots/`配下に保存し、Readツールで直接確認する。
...
```bash
node .claude/skills/run-hiranoa/driver.mjs eval \
...
| `quit`が`"PID xxx が見つかりませんでした"`で失敗する／CDPが応答し続ける | `.driver-pid`のPIDが指すプロセスツリーが（別の`launch`との混在や中間プロセスの異常終了で）既に切れ、`hiranoa.exe`本体が孤児化している可能性がある。`Get-CimInstance Win32_Process -Filter "Name='msedgewebview2.exe'"`で`--user-data-dir`が`.claude/skills/run-hiranoa/.webview2-profile`（自分専用プロファイル）を指しているものを探し、その最上位（`--type=`が付かない、`hiranoa.exe`を親に持つ）プロセスIDに対して`taskkill /PID <id> /T /F`を実行する。`--user-data-dir`が`com.cocoabreak.hiranoa\EBWebView`（既定パス）のものは別プロセス（手動起動や他ツールからの起動）の可能性があるため触らないこと。 |
