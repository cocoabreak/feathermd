...
- `WorkingSet64`: Windowsがその時点でプロセスのワーキングセットへ割り当てている物理メモリ量。共有ページを含み得るため、物理メモリの排他的所有量ではない。
- `PrivateMemorySize64`: 他プロセスと共有できない、プロセス固有のコミット済みメモリ量。ワーキングセットとは意味が異なり、相互に加算しない。

起動・描画時間へ安定待ちを混入させないため、メモリはtiming suiteとは別の隔離済みrelease suiteで計測する。各シナリオで250ms間隔のプロセススナップショットを2回採取し、PID、親PID、作成時刻、実行ファイルが一致する場合だけ2回目の値を採用する。各スナップショット内でもCIMのidentityと`Get-Process`のメモリ値をPIDだけで結合せず、両者の作成時刻をCIMのマイクロ秒精度で照合する。専用WebViewプロファイルをコマンドラインに持つ全WebView2プロセスがTauri子孫ツリー内にあることも必須とする。PID消失・再利用、重複PID、取得不能な個別値、ツリー外へ移動した所有WebView2、スナップショット間の構成変化は理由コード付きの`not-measured`とする。
...
### CIサイズ上限（2026-08-10、#38）

3回の自然変動で観測した最大0.027%に対し、機能追加を伴わない依存更新やchunk分割の微小変化を許容しつつ、大幅な回帰を検出する上限として、初回ベースラインから5%増を採用する。上限は各baseline値に1.05を乗じてbyte単位で切り上げる。総出力、初期ロード、主要機能グループのraw/Brotliをすべてゲート対象にする。

| 対象 | raw baseline | raw上限 | Brotli baseline | Brotli上限 |
| --- | ---: | ---: | ---: | ---: |
| 総Vite出力 | 8,734,210 B | 9,170,921 B | 2,379,635 B | 2,498,617 B |
| 初期ロード | 200,914 B | 210,960 B | 59,014 B | 61,965 B |
| KaTeX機能グループ | 1,908,215 B | 2,003,626 B | 1,054,897 B | 1,107,642 B |
| Mermaid機能グループ | 3,450,733 B | 3,623,270 B | 823,514 B | 864,690 B |
| Shiki機能グループ | 2,393,647 B | 2,513,330 B | 230,118 B | 241,624 B |

レビュー対象のポリシーは `app/perf/size-budget.json` に置き、baseline commit、計測回数、観測変動、許容率、全baseline値を明示する。`perf:ci` は計測artifactを書いた後に全10指標を評価し、1件でも上限を超えた場合は予算JSON/Markdownを残して失敗する。ポリシーの欠損・未知指標・不正値、現在値の欠損・分類不能もfail-closedに扱う。上限更新はこのポリシーファイルと根拠の同時変更を必要とし、時間・メモリは引き続きCIゲートへ含めない。
