---
name: run-hiranoa
description: Build, launch, and drive the Hiranoa Tauri desktop app (WebView2) via Chrome DevTools Protocol — take screenshots, click or drag elements, dispatch keyboard shortcuts, evaluate JS, open files without the native file dialog. Use when asked to run, start, screenshot, or verify a UI change in this app.
---
...
**このファイルのパスはリポジトリルート（`hiranoa/`）基準。** ドライバーは
`.agents/skills/run-hiranoa/driver.mjs`。CDPの共通実装は
`app/scripts/webview2-driver.mjs`にあり、自動スモークテストも同じ実装を使う。

## 前提

- **本スキルはWindows専用**（WebView2のCDPポート・`taskkill`前提。LinuxのWebKitGTK / macOSのWKWebViewはCDP非対応）。CIでは同じCDP実装を使う`app/e2e/smoke.mjs`をWindows runnerで実行する。
...
node .agents/skills/run-hiranoa/driver.mjs launch
# → "CDP準備完了。" が出るまで待つ（初回Rustビルドがあると数分かかることがある）

# 2. スクリーンショット
node .agents/skills/run-hiranoa/driver.mjs screenshot .agents/skills/run-hiranoa/screenshots/foo.png

# 3. ネイティブファイルダイアログを介さずファイルを開く（devビルド限定フック、後述）
node .agents/skills/run-hiranoa/driver.mjs openFile "E:/path/to/file.md"

# 4. 要素をクリック（CSSセレクタ）
node .agents/skills/run-hiranoa/driver.mjs click 'button[aria-label="設定"]'

# 5. CDPの実マウス入力で要素間をドラッグ（Pointer Events等の実入力経路を確認）
node .agents/skills/run-hiranoa/driver.mjs mouseDrag '[data-tab-id]:nth-child(1) [data-tab-drag-handle]' '[data-tab-id]:nth-child(2)'

# 6. キーボードショートカットを送る（document.bodyから bubbles:true でdispatchし、window側の
#    keydownリスナーへ届ける。"Ctrl+W" のように "+" 区切り、最後がkey値）
node .agents/skills/run-hiranoa/driver.mjs key "Ctrl+W"

# 7. 任意のJSを評価（Promiseはawaitされ、returnByValueで結果が返る）
node .agents/skills/run-hiranoa/driver.mjs eval 'document.title'

# 8. 終了（起動時に記録したPIDのプロセスツリーをtaskkillで終了）
node .agents/skills/run-hiranoa/driver.mjs quit
```

スクリーンショットは`.agents/skills/run-hiranoa/screenshots/`配下に保存し、画像表示ツールで直接確認する。
...
```bash
node .agents/skills/run-hiranoa/driver.mjs eval \
...
- **既に起動中の同じアプリ（リリースビルド等）があるとCDPが永久にタイムアウトする**。WebView2は既定でアプリ識別子（`com.cocoabreak.hiranoa`）ごとに固定のuser-data-dirを使うため、同一識別子の別インスタンスが先に起動しているとプロファイルが排他ロックされ、`driver.mjs launch`したプロセスのWebViewが初期化されずCDPポートも開かない（`hiranoa.exe`プロセス自体は起動するが`msedgewebview2.exe`の子プロセスが一切生えない状態になる）。`launch`は`WEBVIEW2_USER_DATA_FOLDER`で`.agents/skills/run-hiranoa/.webview2-profile/`という専用ディレクトリを指定することでこれを回避している。もし手動でこの環境変数を外して再現したくなった場合の切り分け: `Get-CimInstance Win32_Process -Filter "Name='msedgewebview2.exe'"`でその`hiranoa.exe`のPIDを親に持つプロセスが存在するか確認する。
...
| `quit`が`"PID xxx が見つかりませんでした"`で失敗する／CDPが応答し続ける | `.driver-pid`のPIDが指すプロセスツリーが（別の`launch`との混在や中間プロセスの異常終了で）既に切れ、`hiranoa.exe`本体が孤児化している可能性がある。`Get-CimInstance Win32_Process -Filter "Name='msedgewebview2.exe'"`で`--user-data-dir`が`.agents/skills/run-hiranoa/.webview2-profile`（自分専用プロファイル）を指しているものを探し、その最上位（`--type=`が付かない、`hiranoa.exe`を親に持つ）プロセスIDに対して`taskkill /PID <id> /T /F`を実行する。`--user-data-dir`が`com.cocoabreak.hiranoa\EBWebView`（既定パス）のものは別プロセス（手動起動や他ツールからの起動）の可能性があるため触らないこと。 |
