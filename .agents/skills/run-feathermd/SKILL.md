---
name: run-feathermd
description: Build, launch, and drive the FeatherMD Tauri desktop app (WebView2) via Chrome DevTools Protocol — take screenshots, click or drag elements, dispatch keyboard shortcuts, evaluate JS, open files without the native file dialog. Use when asked to run, start, screenshot, or verify a UI change in this app.
---
...
`.agents/skills/run-feathermd/driver.mjs`。CDPの共通実装は
...
node .agents/skills/run-feathermd/driver.mjs launch
# → "CDP準備完了。" が出るまで待つ（初回Rustビルドがあると数分かかることがある）

# 2. スクリーンショット
node .agents/skills/run-feathermd/driver.mjs screenshot .agents/skills/run-feathermd/screenshots/foo.png

# 3. ネイティブファイルダイアログを介さずファイルを開く（devビルド限定フック、後述）
node .agents/skills/run-feathermd/driver.mjs openFile "E:/path/to/file.md"

# 4. 要素をクリック（CSSセレクタ）
node .agents/skills/run-feathermd/driver.mjs click 'button[aria-label="設定"]'

# 5. CDPの実マウス入力で要素間をドラッグ（Pointer Events等の実入力経路を確認）
node .agents/skills/run-feathermd/driver.mjs mouseDrag '[data-tab-id]:nth-child(1) [data-tab-drag-handle]' '[data-tab-id]:nth-child(2)'

# 6. キーボードショートカットを送る（document.bodyから bubbles:true でdispatchし、window側の
#    keydownリスナーへ届ける。"Ctrl+W" のように "+" 区切り、最後がkey値）
node .agents/skills/run-feathermd/driver.mjs key "Ctrl+W"

# 7. 任意のJSを評価（Promiseはawaitされ、returnByValueで結果が返る）
node .agents/skills/run-feathermd/driver.mjs eval 'document.title'

# 8. 終了（起動時に記録したPIDのプロセスツリーをtaskkillで終了）
node .agents/skills/run-feathermd/driver.mjs quit
```

スクリーンショットは`.agents/skills/run-feathermd/screenshots/`配下に保存し、画像表示ツールで直接確認する。
...
```bash
node .agents/skills/run-feathermd/driver.mjs eval \
...
| `quit`が`"PID xxx が見つかりませんでした"`で失敗する／CDPが応答し続ける | `.driver-pid`のPIDが指すプロセスツリーが（別の`launch`との混在や中間プロセスの異常終了で）既に切れ、`feathermd.exe`本体が孤児化している可能性がある。`Get-CimInstance Win32_Process -Filter "Name='msedgewebview2.exe'"`で`--user-data-dir`が`.agents/skills/run-feathermd/.webview2-profile`（自分専用プロファイル）を指しているものを探し、その最上位（`--type=`が付かない、`feathermd.exe`を親に持つ）プロセスIDに対して`taskkill /PID <id> /T /F`を実行する。`--user-data-dir`が`com.cocoabreak.feathermd\EBWebView`（既定パス）のものは別プロセス（手動起動や他ツールからの起動）の可能性があるため触らないこと。 |
